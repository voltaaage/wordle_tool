...
function wordsWithCharacters(chars, nonChars) {
  let result = [];
  WORDLE_WORDS.forEach((word) => {
    const charMap = getCharMap(chars);
    const nonCharMap = getCharMap(nonChars);
    let isValid = true;

    word.split('').forEach((char)=> {
      if (nonCharMap.has(char)) {
        isValid = false;
      }
    });
